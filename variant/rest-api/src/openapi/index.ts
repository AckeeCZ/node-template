...
export type OpenAPIRouteRequestBody<T> = T extends {
  post: { requestBody: { content: { 'application/json': infer U } } }
}
  ? U
  : unknown
...
export type OpenAPIRouteHeaderParam<T> = T extends {
  get: { parameters: { header: infer U } }
}
  ? U
  : T extends { post: { parameters: { header: infer U } } }
  ? U
  : unknown
