...
export class NotAuthorized extends HttpJsonError {
    constructor(data: ErrorData = {}, errorData?: any) {
        super(403, data.message || 'Insufficient aceess', data.code || '?', errorData);
    }
}
...
export class NotAuthenticated extends HttpJsonError {
    constructor(data: ErrorData = {}, errorData?: any) {
        super(401, data.message || 'Requires authentication', data.code || '?', errorData);
    }
}

export class NotFound extends HttpJsonError {
    constructor(data: ErrorData = {}, errorData?: any) {
        super(404, data.message || 'Resource not found', data.code || '?', errorData);
    }
}
